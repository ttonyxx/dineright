<template>
  <div>
    <v-card class="mx-auto mt-4" max-width="374" style="overflow: hidden">
...
>>>>>>> Stashed changes
      ...
      <v-card-text>
        <v-row align="center" class="mx-0" style="margin-bottom: -5px">
          <v-chip-group column>
            ...
            <v-dialog v-model="dialog" width="500">
              <template v-slot:activator="{ on, attrs }">
                <v-chip v-bind="attrs" v-on="on" color="var(--main-yellow)"
                  >{{ cafeteria.c.friends.length }} friends</v-chip
                >
              </template>
        ...
        <v-row class="pl-3 pb-4 pt-2" style="height: 150px; overflow: scroll">
          <v-dialog v-model="dialogFood" width="500">
              <template v-slot:activator="{ on, attrs }">
                <div class="menu-item" v-bind = "attrs" v-on = "on" 
                v-for="(food, i) in cafeteria.c.foods" :key="i">
            {{food.name}}<br />
            <v-img
